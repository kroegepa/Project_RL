...
if actor_type == 'uniform_baseline':
    ...
    agent = actor.TabularQActor(environment_train, environment_test,num_episodes=10, epsilon_decay_rate=0.995)
    agent.train()
elif actor_type == 'SMA':
...
with open("tab_q_val_trajecotry.csv", "w") as file:
    writer = csv.writer(file)
    writer.writerow(["price", "hour", "day", "storage", "action", "moving_average"])
    while not terminated:
        ...
        if actor_type == 'tabular_q':
            ...
            writer.writerow([price, hour, day, storage, action, agent.current_average])
        if hour == 24:
